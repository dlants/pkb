...
- `budget`: cap, in US dollars, on the projected cost of a single reindex run. Before any paid embedding/inference work, `reindex` estimates the run's cost from the changed files and per-chunk reuse (no API calls) and aborts when the estimate exceeds the budget, so an unexpectedly large change set must be reindexed locally instead. The projected cost is printed on every run (default $5; a non-positive value disables the gate).
